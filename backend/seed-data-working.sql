-- IMRAS Seed Data - Working Version
...
-- 8. Sample Alerts (using only existing columns)
INSERT INTO alerts (alert_type, message, item_id, severity, status) VALUES 
('low_stock', 'Laptop Computer stock is below reorder point', (SELECT id FROM items WHERE sku = 'ELEC001' LIMIT 1), 'medium', 'active'),
('reorder_required', 'A4 Paper needs to be reordered', (SELECT id FROM items WHERE sku = 'OFF001' LIMIT 1), 'high', 'active');

-- 9. Sample Stock Movements
